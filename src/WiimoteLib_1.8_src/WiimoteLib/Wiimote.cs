...
namespace WiimoteLib
{
	...
	public class Wiimote : IDisposable
	{
		...
		private string mDeviceSerial = string.Empty;

		// unique ID
		private readonly Guid mID = Guid.NewGuid();

		// delegate used for enumerating found Wiimotes
		internal delegate bool WiimoteFoundDelegate(string devicePath, string name, string serial);
		...
		internal Wiimote(string devicePath, string name, string serial)
		{
				...
				if(HIDImports.SetupDiGetDeviceInterfaceDetail(hDevInfo, ref diData, ref diDetail, size, out size, IntPtr.Zero))
				{
					...
					if(HIDImports.HidD_GetAttributes(mHandle.DangerousGetHandle(), ref attrib))
					{
						...
						if(attrib.VendorID == VID && attrib.ProductID == PID)
						{
							...
							if (HIDImports.HidD_GetProductString(mHandle.DangerousGetHandle(), buffer, 126))
								name = Marshal.PtrToStringAuto(buffer);
							
							if (HIDImports.HidD_GetSerialNumberString(mHandle.DangerousGetHandle(), buffer, 126))
								serial = Marshal.PtrToStringAuto(buffer);
							...
							if(!wiimoteFound(diDetail.DevicePath, name, serial))
								break;
						}
					}
					mHandle.Close();
		...
		private bool WiimoteFound(string devicePath, string name, string serial)
		{
			mDevicePath = devicePath;
			mDeviceName = name;
			mDeviceSerial = serial;
		...
		private void InitializeExtension(byte extensionType)
		{
			...
			switch((ExtensionType)type)
			{
				...
				case ExtensionType.MotionPlus:
					...
					this.SetReportType(InputReport.ExtensionAccel, true);
					break;
				default:
...
//				Debug.WriteLine("Wait");
		...
		public string HIDDeviceSerial
		{
			get { return mDeviceSerial; }
		}
